...
SELECT          a.ifdID AS BTicker, a.isincode AS ISINCode, a.ifdename AS EngName, a.ifdname AS ChtName, a.fkind AS AssetType, 
                            '' AS RuleAssetType, '1' AS SaleInTaiwan, '1' AS IsLive, a.icurid AS Currency,
                                ...
                                (SELECT         TOP (1) mvalue
                                  ...
                                  WHERE           (classcod = 'FOF') AND (fkind = 'Oamt') AND (itemcod = a.ifdID) ORDER BY ddate_w DESC) AS FundSize, 0 AS Fundshare,
                                (SELECT          TOP (1) (CASE WHEN isnull(ddate_w, '') = '' THEN NULL ELSE CONVERT(datetime, ddate_w) END) 
                                                              AS Expr1
                                  FROM               isdbTGL.dbo.fn_invest_index AS fn_invest_index_1
                                  WHERE           (classcod = 'FOF') AND (fkind = 'Oamt') AND (itemcod = a.ifdID) ORDER BY ddate_w DESC) AS FundSizeDate, 
                            ...
                            '' AS IndexFundType, 0 AS sharesdecimal, '' AS fkind_blg, '' AS Fund_INDUSTRY_FOCUS, a.compno AS Compno
